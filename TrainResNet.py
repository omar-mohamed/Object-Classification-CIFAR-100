...
with graph.as_default():
    ...
    with tf.control_dependencies(update_ops):
        ...
        learning_rate = tf.train.exponential_decay(0.0005, global_step, train_size / batch_size, 0.98, staircase=True)
...
early_stop_counter = 10  # stop if test loss is not decreasing for early_stop_counter iterations
...
with tf.Session(graph=graph, config=tf.ConfigProto(log_device_placement=True)) as session:
    ...
    train_conv_keep_prob = 1
    train_conv_keep_prob_min = 0.80
